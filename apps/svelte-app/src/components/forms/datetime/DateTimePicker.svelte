...
<script lang="ts">
  ...
  const handleTimeChange = (e: CustomEvent) => {
    let date = e.detail;
    if (rangeFrom !== null && dayjs(date).isBefore(rangeFrom, 'hour')) {
    ...
    if (rangeTo !== null && dayjs(date).isAfter(rangeTo, 'hour')) {
      value.setHours(time.getHours())
      showTime = 0;
      timeoutID = setTimeout(() => (showTime = 1), 100);
      return
...
<div class={`date-time-picker-${id} relative w-48`}>
  <button class="flex items-center border border-gray-300 bg-white rounded-md cursor-pointer" on:click={togglePicker}>
    <input
      {id}
      class="border-0 p-2 w-40 bg-transparent focus:border-none focus:outline-none box-shadow-none"
      readonly
      type="text"
    ...
    <div class="flex pr-2">
      <Icon i="dashboard" class="h-4 w-4 fill-black" />
    </div>
  </button>
  {#if showPicker}
    <div class="absolute left-0 mt-2 bg-white border border-gray-200 rounded-md shadow-lg z-10">
      ...
      <div class="shadow-md">
        {#if $selectedTab === 'calendar'}
          <DatePicker bind:value min={rangeFrom} max={rangeTo} />
        {:else if showTime !== 0}
          <TimePicker
            bind:date={value}
