...
<script lang="ts">
  ...
  const role = field('role', $modal.id, []);
  const permissions = field('permissions', getDefault(), []);
  const modalForm = form(role, permissions);

  function getDefault() {
    return permissionOptions.reduce(
      (r, p) => ({
        [p.toLowerCase()]: $modal.source.permissions.find((s: Permission) => s.scope === p.toLowerCase()).level,
        ...r,
      }),
      {},
    );
...
<FormModal {formId} show={$modal.event === 'edit'} disabled={$modalForm.errors.length != 0}>
  ...
  <form id={formId} class="grid grid-cols-1 gap-4" on:submit|preventDefault={handleEdit}>
    <FormControl name="Role name">
      <TextInput name="Role name" maxlength={32} value={$role.value} disabled />
