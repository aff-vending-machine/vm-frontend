...
<div class="flex justify-center space-x-4">
  <button on:click|stopPropagation={handleAction('done')} class="focus:outline-none" {disabled}>
    <Icon
      class="w-4 h-4 {disabled ? 'fill-gray-500 hover:fill-gray-500' : 'fill-green-500 hover:fill-green-700'}"
      i="ic-done"
    />
  </button>
  <button on:click|stopPropagation={handleAction('cancel')} class="focus:outline-none" {disabled}>
    <Icon
      class="w-4 h-4 {disabled ? 'fill-gray-500 hover:fill-gray-500' : 'fill-red-500 hover:fill-red-700'}"
      i="ic-cancel"
    />
  </button>
</div>
